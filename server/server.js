...
app.use("/api/insikter", apiPublicLimiter, insightsRouter);
...
app.get("/api/ssr/static/:key", async (req, res) => {
  try {
    const { renderStaticHtml, renderJobsHubHtml, renderHomeHtml } = await import("./lib/seoRender.js");
    const html = req.params.key === "jobb"
      ? await renderJobsHubHtml()
      : req.params.key === "home"
        ? await renderHomeHtml()
        : renderStaticHtml(req.params.key);
