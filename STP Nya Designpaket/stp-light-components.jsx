/* ════════════════════════════════════════════════════════════
   STP — Ljust tema: delade React-komponenter
   
   Importera EFTER React + Babel, och EFTER stp-light-tokens.css:
     ...
     <script type="text/babel" src="...din skärm..."></script>

   Komponenter exporteras till window i slutet — ditt skärm-skript
   kan referera dem direkt (Pill, Button, Card, ...)
════════════════════════════════════════════════════════════ */

const { useState, useEffect, useRef } = React;

/* ────── Icon ────── */
const Icon = ({ name, size = 16, color = "currentColor", stroke = 1.8, style }) => {
  const paths = {
    ...
    menu: <><line x1="3" y1="6" x2="21" y2="6"/><line x1="3" y1="6" x2="21" y2="6"/><line x1="3" y1="12" x2="21" y2="12"/><line x1="3" y1="18" x2="21" y2="18"/></>,
...
/* ────── Pill / Tag ────── */
const Pill = ({ tone = "neutral", children, icon, onRemove, size = "md" }) => {
  const tones = {
    primary: {
      bg: "var(--green)", color: "#fff", border: "var(--green)",
      shadow: "0 1px 2px rgba(31,95,92,0.20), inset 0 -1px 0 rgba(0,0,0,0.10)",
    },
    ...
    <span style={{
      ...
      {onRemove && (
        <button onClick={onRemove} style={{
          color: "inherit", opacity: 0.75, marginLeft: 2,
...
/* ────── Button ────── */
const Button = ({ children, variant = "primary", size = "md", icon, iconRight, onClick, disabled, style, type = "button", full }) => {
  const variants = {
...
/* ────── Card ────── */
const Card = ({ children, padding = "26px 28px", style, className = "" }) => (
...
const SectionLabel = ({ children, accessory, style }) => (
  <header style={{
...
const Field = ({ label, value, mono, action }) => (
  <div style={{
    padding: "12px 0",
    ...
    <div style={{ flex: 1, minWidth: 0 }}>
      <p style={{
        fontSize: 11, color: "var(--ink-500)", marginBottom: 4,
        letterSpacing: 0.3, fontWeight: 500,
      }}>{label}</p>
      <p style={{
        fontSize: 15, color: "var(--ink-900)", fontWeight: 600,
...
/* ────── Tabs ────── */
const Tabs = ({ value, onChange, items, style }) => (
...
/* ────── TopNav — dark asfalt strip ────── */
const TopNav = ({ items, active, onActive, currentUser, brand = "STP", brandSub, rightExtras, sticky = true }) => (
  <nav style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginRight: 28 }}>
        ...
        {brandSub && (
          <span style={{
            ...
            paddingLeft: 8, marginLeft: 4, borderLeft: "1px solid rgba(255,255,255,0.15)",
...
/* ────── PageShell — paper bg + max-width content area ────── */
const PageShell = ({ children, maxWidth = 1240 }) => (
  <div style={{ minHeight: "100vh", background: "var(--paper)" }}>
    {children}
  </div>
);

const Container = ({ children, maxWidth = 1240, style, padding = "32px 32px 0" }) => (
  <div style={{ maxWidth, margin: "0 auto", padding, ...style }}>{children}</div>
);

/* ────── Input ────── */
const Input = ({ label, hint, error, icon, type = "text", ...rest }) => (
  <label style={{ display: "flex", flexDirection: "column", gap: 6 }}>
    {label && (
      <span style={{ fontSize: 12.5, fontWeight: 600, color: "var(--ink-700)", letterSpacing: 0.1 }}>{label}</span>
    )}
    <div style={{
      display: "flex", alignItems: "center", gap: 8,
      padding: "0 12px",
...
const Divider = ({ style }) => (
  <div style={{
    height: 1, background: "var(--line)", margin: "8px 0",
    ...style,
  }}/>
);

/* ────── Stat — big number + label ────── */
const Stat = ({ value, label, tone = "ink", mono = true, trend }) => {
  const colors = {
    ink:     "var(--ink-900)",
    primary: "var(--green)",
    success: "var(--success)",
  ...
  return (
    <div>
      <div style={{
        ...
        color: colors[tone], letterSpacing: -0.5,
        fontFamily: mono ? "var(--mono)" : "var(--font)",
      }}>{value}</div>
      <div style={{
        fontSize: 12, color: "var(--ink-500)", marginTop: 5,
        fontWeight: 600, letterSpacing: 0.2, textTransform: "uppercase",
      }}>{label}</div>
      {trend && (
        <div style={{ fontSize: 12, color: trend.startsWith("+") ? "var(--success)" : "var(--danger)", marginTop: 3, fontWeight: 600 }}>
          {trend}
        </div>
      )}
    </div>
...
/* ────── Notice — inline informational box ────── */
const Notice = ({ tone = "info", title, children, action, icon }) => {
  const tones = {
  ...
  return (
    <div style={{
      ...
      {icon !== false && (
        <Icon name={s.iconName} size={18} color={s.color} stroke={1.8} style={{ flexShrink: 0, marginTop: 1 }}/>
      )}
...
/* ────── Avatar ────── */
const Avatar = ({ initials, size = 40, color = "var(--green)", style }) => (
  <div style={{
    width: size, height: size, borderRadius: "50%", flexShrink: 0,
    background: color, color: "#fff",
    fontSize: size * 0.34, fontWeight: 800, letterSpacing: 0.3,
    display: "flex", alignItems: "center", justifyContent: "center",
    ...style,
  }}>{initials}</div>
);
...
Object.assign(window, {
  Icon, Pill, Dot, Button, Card, SectionLabel, Field, Tabs,
  TopNav, PageShell, Container, Input, Divider, Stat, Notice, Avatar,
});
