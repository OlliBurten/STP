...
export function VerificationSheet({ ctx, close }) {
  ...
  return (
    <div style={{ padding: "4px 22px 26px" }}>
      ...
      <p style={{ fontSize: 14.5, color: "var(--ink-600)", lineHeight: 1.55, marginBottom: 16 }}>{verified ? "Er profil visar en grön Verifierad av STP-stämpel. Förare litar på verifierade åkerier." : "Vi verifierar automatiskt mot Bolagsverket via ert organisationsnummer – är ni registrerade som transport-/åkeriföretag (SNI) blir ni verifierade direkt."}</p>
      ...
      {!verified && <div style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "13px 14px", background: "var(--info-tint)", borderRadius: 12, marginBottom: 18 }}><Icon name="info" size={17} color="var(--info)" stroke={2} style={{ flexShrink: 0, marginTop: 1 }} /><span style={{ fontSize: 13, color: "var(--ink-700)", lineHeight: 1.45 }}>Vi kunde inte automatiskt bekräfta att org.nr {ctx.company.orgnr || "—"} är ett transport-/åkeriföretag. Mejla <b>hello@transportplattformen.se</b> så hjälper vi er.</span></div>}
