...
export default function CompanyChatScreen({ ctx }) {
  ...
  const messages = (conv?.messages || []).map((m) => ({ me: m.sender === "company", text: m.content, t: timeAgo(m.timestamp) }));
  useEffect(() => { if (endRef.current) endRef.current.scrollTo(0, 99999); }, [messages.length]);

  if (!conv) return null;
  const send = () => { const text = val.trim(); if (!text) return; ctx.chat?.sendMessage?.(conv.id, text, "company"); setVal(""); };
