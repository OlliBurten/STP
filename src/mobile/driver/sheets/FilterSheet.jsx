...
export default function FilterSheet({ ctx, close }) {
  ...
  const [hideBem, setHideBem] = useState(Boolean(ctx.filter.hideBemanning));
  const toggleLic = (l) => setLic((s) => (s.includes(l) ? s.filter((x) => x !== l) : [...s, l]));
  const toggleCert = (c) => setCert((s) => (s.includes(c) ? s.filter((x) => x !== c) : [...s, c]));
  const apply = () => { ctx.setFilter({ type, lic, cert, hideBemanning: hideBem }); close(); };
  const clear = () => { setType("alla"); setLic([]); setCert([]); setHideBem(false); ctx.setFilter({ type: "alla", lic: [], cert: [], hideBemanning: false }); close(); };
  ...
  const matchCount = (ctx.jobs || []).filter((j) => {
    ...
    if (hideBem && j.bemanning) return false;
    return true;
  }).length;
  ...
  return (
    <div style={{ padding: "0 22px 26px" }}>
      ...
      <Label style={{ marginBottom: 10 }}>Anställning</Label>
      <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 24 }}>
        {chip(hideBem, "Dölj bemanning", () => setHideBem((v) => !v))}
      </div>
