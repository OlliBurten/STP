...
import { Header, ScrollArea, Card, Empty, Button } from "../../ui";
        ...
        <div style={{ padding: "6px 16px 24px" }}>
          {threads.length === 0 ? (
            ...
            <Card style={{ padding: "0 16px", overflow: "hidden" }}>
              {threads.map((t, i) => (
                <button key={t.id} onClick={() => ctx.setChat(t.conv)} className="press" style={{ width: "100%", display: "flex", alignItems: "center", gap: 13, padding: "15px 0", borderBottom: i < threads.length - 1 ? "1px solid var(--line)" : "none", textAlign: "left" }}>
