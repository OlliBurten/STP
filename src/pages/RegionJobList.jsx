    ...
    <div style={{ minHeight: "100vh", background: "var(--paper)" }}>
      ...
      <div style={{ background: "var(--card)", borderBottom: "1px solid var(--line)", padding: isMobile ? "24px 20px 20px" : "40px 40px 32px" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          ...
          {/* Breadcrumb */}
          <nav style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: "var(--ink-400)", marginBottom: 18 }}>
            <Link to="/jobb" style={{ color: "var(--ink-400)", textDecoration: "none" }}>Alla jobb</Link>
            <span>›</span>
            <span style={{ color: "var(--ink-500)" }}>{region.name}</span>
          </nav>

          <div style={{ fontSize: 11, fontWeight: 800, color: "var(--green-text)", letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 8 }}>
            Region
          </div>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: isMobile ? "24px 20px" : "32px 40px" }}>
        ...
        {loading ? (
          ...
          <div style={{ borderRadius: 18, border: "1px solid var(--line)", background: "var(--card)", padding: "48px 24px", textAlign: "center", marginBottom: 40 }}>
            <p style={{ color: "var(--ink-400)", fontSize: 14, marginBottom: 20 }}>
              Inga lediga tjänster i {region.name} just nu.
            </p>
            <Link
              to="/jobb"
              style={{ display: "inline-block", background: "var(--green)", color: "#fff", padding: "10px 20px", borderRadius: 10, fontSize: 13, fontWeight: 700, textDecoration: "none" }}
            >
              Sök i hela Sverige →
            </Link>
          </div>
        ...
        <div style={{ borderRadius: 18, background: "var(--green-tint)", border: "1px solid rgba(31,95,92,0.2)", padding: isMobile ? "24px 20px" : "28px 32px", display: "flex", flexDirection: isMobile ? "column" : "row", alignItems: isMobile ? "flex-start" : "center", gap: 20, marginBottom: 40 }}>
          <div style={{ flex: 1 }}>
            <h3 style={{ fontSize: 16, fontWeight: 700, color: "var(--ink-900)", margin: "0 0 4px" }}>Åkeri i {region.name}?</h3>
          ...
          <Link
            ...
            style={{ flexShrink: 0, display: "inline-block", background: "var(--green)", color: "#fff", padding: "10px 20px", borderRadius: 10, fontSize: 13, fontWeight: 700, textDecoration: "none", whiteSpace: "nowrap" }}
          >
        ...
        <div style={{ borderTop: "1px solid var(--line)", paddingTop: 28 }}>
          <h2 style={{ fontSize: 14, fontWeight: 700, color: "var(--ink-500)", marginBottom: 14 }}>
            ...
            {regionPages
              ...
              .map((r) => (
                <Link
                  ...
                  style={{ padding: "6px 14px", borderRadius: 8, border: "1px solid var(--line)", fontSize: 13, color: "var(--ink-500)", textDecoration: "none", transition: "border-color .15s, color .15s" }}
