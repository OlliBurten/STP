...
const SEGMENTS = [
  { Icon: TruckIcon, title: "Heltid", tag: "Fast anställning", body: "För dig som söker en fast, långsiktig tjänst.", points: ["Fasta tjänster", "Långsiktiga åkerier"] },
  { Icon: ClockIcon, title: "Vikarie & deltid", tag: "Flexibelt", body: "Vikariat, extrapass och kortare behov.", points: ["Extrapass", "Kortare uppdrag"] },
  { Icon: BuildingIcon, title: "Praktik", tag: "I början av karriären", body: "För dig som är elev eller ny i yrket.", points: ["Praktikplatser", "Mentorskap"] },
];
...
const FAQS = [
  { q: "Måste jag vara yrkesförare?", a: "Ja — STP är byggt för yrkesförare (B, C, CE, C1). YKB och ADR hanteras direkt." },
  { q: "Kan jag vara anonym tills jag tar kontakt?", a: "Ja. Du styr din synlighet och kan stänga av den när som helst." },
  { q: "Tar STP betalt av förare?", a: "Nej. STP är alltid gratis för förare — skapa profil, söka jobb och bli kontaktad kostar aldrig något." },
      ...
      <div style={{ padding: "0 24px 24px", position: "relative", zIndex: 1 }}>
        ...
        <div style={{ background: "var(--card-2)", border: "1px solid var(--line)", borderRadius: 13, padding: "14px 16px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            <span style={{ fontSize: "var(--text-xs)", fontWeight: 700, color: "var(--ink-700)" }}>Profilstyrka</span>
            <span style={{ fontSize: "var(--text-sm)", fontWeight: 800, color: "var(--green-text)", fontFamily: "var(--mono)" }}>92%</span>
          </div>
          ...
          <div style={{ fontSize: "var(--text-2xs)", color: "var(--ink-400)", marginTop: 9, lineHeight: 1.4 }}>Kompletta profiler syns oftare i åkeriernas sök.</div>
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 12, padding: "12px 14px", background: "var(--green-tint)", border: "1px solid rgba(30,107,91,0.14)", borderRadius: 13 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: "var(--text-2xs)", fontWeight: 800, color: "var(--green-text)", textTransform: "uppercase", letterSpacing: 0.9, marginBottom: 2 }}>Ny matchning</div>
            <div style={{ fontSize: "var(--text-base)", fontWeight: 800, color: "var(--ink-900)", whiteSpace: "nowrap" }}>Nordisk Frakt · Fjärr</div>
          </div>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "5px 10px", borderRadius: 999, background: "var(--success-tint)", flexShrink: 0 }}>
            <span style={{ fontSize: "var(--text-sm)", fontWeight: 800, fontFamily: "var(--mono)", color: "var(--success)" }}>94%</span>
...
const VisRow = ({ label, on }) => (
  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "13px 0", borderBottom: "1px solid var(--line)" }}>
    <span style={{ fontSize: "var(--text-base)", color: "var(--ink-700)", fontWeight: 500 }}>{label}</span>
                ...
                {DEMAND.map((d) => (
                  <div key={d.region}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 7 }}>
                      <span style={{ fontSize: "var(--text-base)", fontWeight: 700, color: "var(--ink-900)" }}>{d.region}</span>
                      <span style={{ fontSize: "var(--text-sm)", fontWeight: 700, color: "var(--ink-500)", fontFamily: "var(--mono)" }}>{d.n} tjänster</span>
                    </div>
            ...
            <Card padding="26px 28px" style={{ boxShadow: "var(--sh)" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <span style={{ fontSize: "var(--text-md)", fontWeight: 800, color: "var(--ink-900)" }}>Synlighet</span>
                </div>
              ...
              <p style={{ fontSize: "var(--text-xs)", color: "var(--ink-400)", marginTop: 14, lineHeight: 1.5 }}>
              ...
              <div style={{ display: "grid", gap: 12 }}>
                {["Var anonym tills du själv tar kontakt", "Stäng av synligheten med ett klick", "Bestäm vad som är publikt och vad som bara används för matchning"].map((t) => (
                  <div key={t} style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                    ...
                    <span style={{ fontSize: "var(--text-md)", color: "var(--ink-700)", lineHeight: 1.55 }}>{t}</span>
                  </div>
                ))}
              </div>
            </div>
              ...
              <Card key={s.title} padding="0" style={{ overflow: "hidden" }}>
                <div style={{ padding: "26px 26px 24px" }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
                    ...
                    <span style={{ fontSize: "var(--text-2xs)", fontWeight: 700, color: "var(--ink-400)", textTransform: "uppercase", letterSpacing: 0.8 }}>{s.tag}</span>
                  </div>
                  <h3 style={{ fontSize: "var(--text-2xl)", fontWeight: 800, color: "var(--ink-900)", marginBottom: 9, letterSpacing: -0.4 }}>{s.title}</h3>
                  <p style={{ fontSize: "var(--text-base)", color: "var(--ink-500)", lineHeight: 1.65, textWrap: "pretty" }}>{s.body}</p>
                </div>
                <div style={{ borderTop: "1px solid var(--line)", padding: "14px 26px", display: "flex", gap: 14, background: "var(--card-2)" }}>
                  {s.points.map((p) => (
                    <span key={p} style={{ fontSize: "var(--text-xs)", color: "var(--ink-700)", fontWeight: 600, display: "inline-flex", alignItems: "center", gap: 6 }}>
