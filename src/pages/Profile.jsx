...
function Tag({ c = "p", children, onRemove }) {
  ...
  return (
    <span style={{
      ...
      padding: "3px 10px", borderRadius: 20, fontSize: "var(--text-xs)", fontWeight: 600,
      ...
      {onRemove && (
        <button onClick={onRemove} style={{
          ...
          color: s.color, opacity: 0.7, padding: 0, lineHeight: 1, fontSize: "var(--text-sm)",
        }}>×</button>
      )}
    </span>
  );
...
function SectionHeader({ label, action }) {
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
      <p style={{ fontSize: "var(--text-2xs)", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: T.sub }}>{label}</p>
      {action}
    </div>
  );
}
  ...
  return (
    <Card style={{ marginBottom: 14 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
        <p style={{ fontSize: "var(--text-sm)", fontWeight: 700, color: T.text }}>Profilstyrka</p>
        <span style={{ fontSize: "var(--text-2xl)", fontWeight: 900, color: barColor }}>
          {score}<span style={{ fontSize: "var(--text-xs)", fontWeight: 400, color: T.sub }}>/100</span>
        </span>
      </div>
      ...
      <p style={{ fontSize: "var(--text-xs)", color: T.sub, marginBottom: 14 }}>{label}</p>
      ...
      {checks.every((c) => c.done) ? (
        ...
        <button onClick={onEdit} style={{
          ...
          background: T.pDim, color: "var(--green-text)", fontSize: "var(--text-xs)", fontWeight: 700,
...
function MarketSidebar({ driverMarket, user, linkCopied, onCopyLink, profileStats, certExpiry }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      {profileStats && (
        <Card>
          <p style={{ fontSize: "var(--text-2xs)", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: T.sub, marginBottom: 14 }}>
            Profilvisningar
          </p>
          ...
          {profileStats.conversationCount > 0 && (
            <p style={{ fontSize: "var(--text-xs)", color: T.green, fontWeight: 600, margin: 0 }}>
          ...
          {(profileStats.views7 ?? 0) === 0 && (profileStats.views30 ?? 0) === 0 && (
            <p style={{ fontSize: "var(--text-2xs)", color: T.muted, lineHeight: 1.5, margin: 0 }}>
      ...
      {certWarnings.length > 0 && (
        <Card style={{ border: `1px solid var(--amber-tint-2)`, background: "var(--amber-tint)" }}>
          <p style={{ fontSize: "var(--text-2xs)", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: T.amber, marginBottom: 12 }}>
            ⚠ Certifikat löper ut
          </p>
          {certWarnings.map(({ id, daysLeft }) => (
            <div key={id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
              <span style={{ fontSize: "var(--text-xs)", color: T.sub }}>{id}</span>
              <span style={{ fontSize: "var(--text-2xs)", fontWeight: 700, color: daysLeft < 30 ? T.red : T.amber }}>
                {daysLeft} dagar
              </span>
            </div>
          ))}
          <p style={{ fontSize: "var(--text-2xs)", color: T.muted, marginTop: 6, lineHeight: 1.5 }}>
        ...
        <Card>
          ...
          {driverMarket.topLicenses.map((l) => (
            <div key={l.name} style={{ marginBottom: 11 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                <span style={{ fontSize: "var(--text-xs)", color: T.sub }}>{l.name}</span>
                <span style={{ fontSize: "var(--text-xs)", fontWeight: 600, color: T.text }}>{l.pct}%</span>
              </div>
          ...
          {driverMarket.topCerts.slice(0, 3).map((c) => (
            <div key={c.name} style={{ marginBottom: 11 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                <span style={{ fontSize: "var(--text-xs)", color: T.sub }}>{c.name}</span>
                <span style={{ fontSize: "var(--text-xs)", fontWeight: 600, color: T.text }}>{c.pct}%</span>
              </div>
              <div style={{ height: 4, borderRadius: 4, background: "var(--paper-2)" }}>
          ...
          <p style={{ fontSize: "var(--text-2xs)", color: T.muted, marginTop: 6 }}>Andel aktiva jobb som kräver respektive</p>
        </Card>
      )}
      ...
      {user?.id && (
        <Card>
          <p style={{ fontSize: "var(--text-2xs)", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: T.sub, marginBottom: 12 }}>
            Din profillänk
          </p>
          <p style={{ fontSize: "var(--text-xs)", color: T.sub, marginBottom: 12, lineHeight: 1.5 }}>
          ...
          <div style={{ padding: "8px 12px", borderRadius: 8, background: T.card, border: `1px solid ${T.border}`, marginBottom: 10, overflow: "hidden" }}>
            <p style={{ fontSize: "var(--text-2xs)", color: T.muted, fontFamily: "monospace", wordBreak: "break-all", margin: 0 }}>
          ...
          <button onClick={onCopyLink} style={{
            ...
            background: "transparent", color: T.text, fontSize: "var(--text-sm)", fontWeight: 600,
...
function CertSuggestionInline({ token }) {
  ...
  if (status === "sent") {
    return <p style={{ marginTop: 8, fontSize: "var(--text-xs)", color: T.green }}>Tack! Vi har tagit emot ditt förslag.</p>;
  }
    ...
    <div style={{ marginTop: 8 }}>
      {!open ? (
        <button type="button" onClick={() => setOpen(true)} style={{
          fontSize: "var(--text-xs)", color: T.muted, background: "none", border: "none", cursor: "pointer",
        ...
        <form onSubmit={submit} style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <input
            ...
            style={{
              ...
              color: T.text, fontSize: "var(--text-xs)", fontFamily: T.font, outline: "none",
            }}
          />
          <button type="submit" disabled={status === "sending" || text.trim().length < 2} style={{
            padding: "7px 14px", borderRadius: 8, border: "none",
            background: T.primary, color: "#fff", fontSize: "var(--text-xs)", fontWeight: 600,
          ...
          <button type="button" onClick={() => { setOpen(false); setStatus(null); setText(""); }} style={{
            fontSize: "var(--text-xs)", color: T.muted, background: "none", border: "none", cursor: "pointer", fontFamily: T.font,
          }}>Avbryt</button>
          {status === "error" && <span style={{ fontSize: "var(--text-xs)", color: T.red }}>Något gick fel.</span>}
        </form>
      )}
    </div>
  );
...
function ExpForm({ initial, onSave, onCancel, isMobile }) {
  ...
  const inputStyle = {
    ...
    background: T.card, border: `1px solid ${T.border2}`,
    color: T.text, fontSize: "var(--text-sm)", fontFamily: T.font, outline: "none",
  };
  const chipBtn = (active) => ({
    padding: "6px 12px", borderRadius: 8, fontSize: "var(--text-xs)", fontWeight: 600, cursor: "pointer",
  ...
  return (
    <div style={{
      ...
      <p style={{ fontSize: "var(--text-sm)", fontWeight: 700, color: T.text, marginBottom: 16 }}>
        {initial ? "Redigera erfarenhet" : "Lägg till erfarenhet"}
      </p>
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 12, marginBottom: 12 }}>
        <div>
          <p style={{ fontSize: "var(--text-xs)", fontWeight: 600, color: T.sub, marginBottom: 6 }}>Företag *</p>
          <input value={d.company} onChange={(e) => upd("company", e.target.value)} placeholder="Nordic Logistics AB" style={inputStyle} />
        </div>
        <div>
          <p style={{ fontSize: "var(--text-xs)", fontWeight: 600, color: T.sub, marginBottom: 6 }}>Roll / titel *</p>
          <input value={d.role} onChange={(e) => upd("role", e.target.value)} placeholder="CE-chaufför" style={inputStyle} />
        </div>
        <div>
          <p style={{ fontSize: "var(--text-xs)", fontWeight: 600, color: T.sub, marginBottom: 6 }}>Start</p>
        ...
        <div>
          <p style={{ fontSize: "var(--text-xs)", fontWeight: 600, color: T.sub, marginBottom: 6 }}>Slut</p>
      ...
      <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: "var(--text-sm)", color: T.sub, marginBottom: 16, cursor: "pointer" }}>
        <input type="checkbox" checked={d.current} onChange={(e) => upd("current", e.target.checked)} />
        Pågående anställning
      </label>
      <div style={{ marginBottom: 12 }}>
        <p style={{ fontSize: "var(--text-xs)", fontWeight: 600, color: T.sub, marginBottom: 8 }}>Fordonstyp</p>
      ...
      <div style={{ marginBottom: 16 }}>
        <p style={{ fontSize: "var(--text-xs)", fontWeight: 600, color: T.sub, marginBottom: 8 }}>Körtyp</p>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
          {EXP_JOB_TYPES.map((j) => {
      ...
      <div style={{ marginBottom: 16 }}>
        <p style={{ fontSize: "var(--text-xs)", fontWeight: 600, color: T.sub, marginBottom: 6 }}>Beskrivning (valfritt)</p>
      ...
      <div style={{ display: "flex", gap: 10 }}>
        <button type="button" onClick={() => onSave(d)} disabled={!d.company || !d.role} style={{
          ...
          background: T.primary, color: "#fff", fontSize: "var(--text-base)", fontWeight: 600,
        ...
        <button type="button" onClick={onCancel} style={{
          ...
          background: "var(--paper-2)", color: T.sub, fontSize: "var(--text-base)", fontWeight: 600,
          cursor: "pointer", fontFamily: T.font,
        }}>Avbryt</button>
      </div>
    </div>
...
export default function Profile() {
  ...
  const chipBtn = (active, colorOverride) => ({
    padding: "7px 14px", borderRadius: 8, fontSize: "var(--text-xs)", fontWeight: 500, cursor: "pointer",
  ...
  const inputStyle = {
    width: "100%", padding: "10px 14px", borderRadius: 9,
    background: T.card, border: `1px solid ${T.border2}`,
    color: T.text, fontSize: "var(--text-base)", fontFamily: T.font, outline: "none",
  };
  ...
  if (isMobile && editing) {
    const mobileInput = {
      ...
      color: "var(--ink-900)", fontSize: "var(--text-md)", fontFamily: "'DM Sans', system-ui, sans-serif",
      outline: "none",
    };
    const SectionLabel = ({ children, id }) => (
      <div id={id} style={{ fontSize: "var(--text-2xs)", fontWeight: 800, letterSpacing: 1.2, textTransform: "uppercase", color: "var(--ink-400)", marginBottom: 10, marginTop: 24 }}>
        {children}
      </div>
    );
      ...
      <div style={{ position: "fixed", inset: 0, background: "var(--paper)", color: "var(--ink-900)", display: "flex", flexDirection: "column", zIndex: 200, fontFamily: "'DM Sans', system-ui, sans-serif" }}>
        ...
        <div style={{ padding: "48px 18px 12px", display: "flex", alignItems: "center", gap: 12, background: "var(--card)", borderBottom: "1px solid var(--line)", flexShrink: 0, boxShadow: "var(--sh-sm)" }}>
          ...
          <button
            ...
            style={{ padding: "8px 16px", borderRadius: 99, background: profileSaving ? "var(--green-tint)" : "var(--green)", border: "none", color: "#fff", fontSize: "var(--text-sm)", fontWeight: 800, cursor: profileSaving ? "default" : "pointer", fontFamily: "inherit" }}
          >
        ...
        {profileSaveError && (
          <div style={{ padding: "10px 18px", background: "var(--danger-tint)", borderBottom: "1px solid var(--danger-tint)", flexShrink: 0 }}>
            <p style={{ fontSize: "var(--text-sm)", color: "var(--danger)", margin: 0 }}>{profileSaveError}</p>
          </div>
        )}
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "4px 20px", paddingBottom: "max(env(safe-area-inset-bottom), 40px)" }}>
          ...
          />
          <p style={{ fontSize: "var(--text-2xs)", color: "rgba(255,255,255,0.35)", marginTop: 6 }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {licenseTypes.map((l) => {
              ...
              return (
                <button key={l.value} onClick={() => toggleLicense(l.value)} style={{ padding: "10px 18px", borderRadius: 99, background: active ? "var(--green)" : "var(--card)", border: `1px solid ${active ? "var(--green-deep)" : "var(--line-2)"}`, color: active ? "#fff" : "var(--ink-700)", fontSize: "var(--text-md)", fontWeight: 800, cursor: "pointer", fontFamily: "inherit", minHeight: 44 }}>
                  {l.label}
                </button>
              );
            })}
          ...
          {((current.licenses || []).includes("C") || (current.licenses || []).includes("CE")) && (
            <p style={{ fontSize: "var(--text-2xs)", color: "var(--ink-400)", marginTop: 6 }}>B ingår automatiskt med C/CE</p>
          )}
          ...
          {certificateGroups.map((group) => (
            <div key={group.id} style={{ marginBottom: 14 }}>
              <div style={{ fontSize: "var(--text-2xs)", color: "var(--ink-500)", fontWeight: 600, marginBottom: 7 }}>{group.label}</div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
                {group.options.map((ct) => {
                  const active = (current.certificates || []).includes(ct.value);
                  return (
                    <button key={ct.value} onClick={() => toggleCertificate(ct.value)} style={{ padding: "8px 13px", borderRadius: 99, background: active ? "var(--success-tint)" : "var(--card)", border: `1px solid ${active ? "var(--success)" : "var(--line-2)"}`, color: active ? "var(--success)" : "var(--ink-700)", fontSize: "var(--text-xs)", fontWeight: 700, cursor: "pointer", fontFamily: "inherit", minHeight: 38 }}>
                      {ct.label}
                    </button>
                  );
                })}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 24 }}>
            {availabilityTypes.map((a) => {
              ...
              return (
                <button key={a.value} onClick={() => updateDraft({ availability: a.value })} style={{ padding: "14px 16px", borderRadius: 12, background: active ? "var(--green-tint)" : "var(--card)", border: `1px solid ${active ? "var(--green-tint-2)" : "var(--line-2)"}`, color: "var(--ink-900)", fontSize: "var(--text-base)", fontWeight: active ? 700 : 500, cursor: "pointer", textAlign: "left", display: "flex", alignItems: "center", gap: 10, fontFamily: "inherit" }}>
  ...
  if (isMobile && !editing) {
    ...
    const RowLink = ({ href, icon, label, value, danger, accent, section }) => (
      <Link to={href || "#"} onClick={href ? undefined : section ? () => startEditingAt(section) : startEditing} style={{ display: "flex", width: "100%", padding: "14px 18px", background: "transparent", border: "none", borderBottom: "1px solid var(--line)", cursor: "pointer", alignItems: "center", gap: 14, minHeight: 54, textDecoration: "none", color: danger ? "var(--danger)" : "var(--ink-900)" }}>
        ...
        <span style={{ flex: 1, fontSize: "var(--text-base)", fontWeight: 600 }}>{label}</span>
        {value && <span style={{ fontSize: "var(--text-xs)", color: "var(--ink-400)" }}>{value}</span>}
      ...
      <div style={{ background: "var(--paper)", minHeight: "100vh", color: "var(--ink-900)", paddingBottom: 100, paddingTop: "calc(env(safe-area-inset-top, 0px) + 20px)", fontFamily: "'DM Sans', system-ui, sans-serif" }}>
        ...
        <div style={{ padding: "0 20px 20px" }}>
          <div style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 18, padding: "20px", textAlign: "center", boxShadow: "var(--sh-sm)" }}>
            ...
            <div style={{ fontSize: "var(--text-sm)", color: "var(--ink-500)", marginBottom: 14 }}>
            ...
            <button onClick={startEditing} style={{ padding: "10px 20px", borderRadius: 99, background: "var(--green-tint)", border: "1px solid var(--green-tint-2)", color: "var(--green-text)", fontSize: "var(--text-sm)", fontWeight: 700, cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 6, minHeight: 40, fontFamily: "inherit" }}>
          ...
          <div style={{ padding: "0 20px 20px" }}>
            <div style={{ background: "var(--amber-tint)", border: "1px solid var(--amber-tint-2)", borderRadius: 13, padding: "14px 16px" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                <span style={{ fontSize: "var(--text-sm)", fontWeight: 700, color: "var(--ink-900)" }}>Profil {progressPct}% klar</span>
                <span style={{ fontSize: "var(--text-2xs)", color: "var(--amber)", fontWeight: 700 }}>{totalSteps - completedSteps} steg kvar</span>
              </div>
        ...
        <div style={{ marginBottom: 20 }}>
          <div style={{ fontSize: "var(--text-2xs)", fontWeight: 800, letterSpacing: 1.2, textTransform: "uppercase", color: "var(--ink-400)", padding: "0 20px", marginBottom: 10 }}>Mina uppgifter</div>
        ...
        {/* Settings */}
        <div style={{ marginBottom: 20 }}>
          <div style={{ fontSize: "var(--text-2xs)", fontWeight: 800, letterSpacing: 1.2, textTransform: "uppercase", color: "var(--ink-400)", padding: "0 20px", marginBottom: 10 }}>Inställningar</div>
          <div style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 14, overflow: "hidden", margin: "0 20px", boxShadow: "var(--sh-sm)" }}>
        ...
        <div style={{ textAlign: "center", padding: "4px 20px 20px", fontSize: "var(--text-2xs)", color: "var(--ink-300)" }}>
  ...
  if (!editing) {
    return (
      <div style={{ minHeight: "100vh", background: "var(--paper)", color: "var(--ink-900)", fontFamily: "var(--font)" }}>
        <div style={{ maxWidth: "var(--w-app)", margin: "0 auto", padding: "28px 32px 80px" }}>
          <DriverProfileView
            profile={current}
            owner={user}
            mode="self"
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--paper)", color: "var(--ink-900)", fontFamily: "var(--font)" }}>
      ...
      {/* ── Hero card ── */}
      <div style={{ maxWidth: "var(--w-app)", margin: "0 auto", padding: "28px 32px 0" }}>
        <div style={{
          background: "var(--card)",
          ...
          {editing && (
            <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 16 }}>
              <span style={{ fontSize: "var(--text-xs)", color: "var(--amber)", fontWeight: 600 }}>● Redigeringsläge — spara längst ned</span>
            </div>
          )}
                ...
                <div style={{ display: "flex", gap: 12, marginBottom: 12, maxWidth: 500 }}>
                  <input
                    ...
                    style={{
                      ...
                      background: "var(--card)", border: "1px solid var(--line-2)",
                      color: "var(--ink-900)", fontSize: "var(--text-2xl)", fontWeight: 700, outline: "none", fontFamily: "var(--font)",
                    }}
                  />
                </div>
              ) : (
                <h1 style={{ fontSize: "var(--text-5xl)", fontWeight: 900, color: "var(--ink-900)", letterSpacing: -1.2, marginBottom: 6 }}>
                  {current.name || "—"}
                </h1>
              )}

              {/* Location + region + years row */}
              <div style={{ display: "flex", alignItems: "center", gap: 8, color: "var(--ink-500)", fontSize: "var(--text-base)", marginBottom: 16 }}>
            ...
            {!editing && (
              <div style={{ display: "flex", gap: 8, flexShrink: 0, paddingTop: 4 }}>
                <button
                  ...
                  style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "8px 16px", borderRadius: 9, border: "1px solid var(--line-2)", background: "var(--card)", color: "var(--ink-700)", fontSize: "var(--text-sm)", fontWeight: 600, cursor: "pointer", fontFamily: "var(--font)" }}
                >
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>
                  {linkCopied ? "Kopierat!" : "Dela profil"}
                </button>
                <button onClick={startEditing} style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "8px 18px", borderRadius: 9, border: "1px solid var(--green)", background: "var(--green)", color: "#fff", fontSize: "var(--text-sm)", fontWeight: 700, cursor: "pointer", fontFamily: "var(--font)" }}>
      ...
      {/* ── Content ── */}
      <div style={{ maxWidth: "var(--w-app)", margin: "0 auto", padding: "20px 32px 80px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 320px", gap: 24, alignItems: "start" }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 18, minHeight: "100vh" }}>
              ...
              <div style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 14, padding: "22px 24px" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
                  <p style={{ fontSize: "var(--text-2xs)", fontWeight: 800, letterSpacing: 1.2, textTransform: "uppercase", color: "var(--ink-500)" }}>Grundläggande</p>
                </div>
                {editing ? (
                  <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                      <div>
                        <p style={{ fontSize: "var(--text-xs)", fontWeight: 600, color: T.sub, marginBottom: 6 }}>Ort</p>
                        <input value={current.location || ""} onChange={(e) => updateDraft({ location: e.target.value })} placeholder="Malmö" style={inputStyle} />
                      </div>
                      <div>
                        <p style={{ fontSize: "var(--text-xs)", fontWeight: 600, color: T.sub, marginBottom: 6 }}>E-post</p>
                        <input type="email" value={current.email || ""} onChange={(e) => updateDraft({ email: e.target.value })} style={inputStyle} />
                      </div>
                      <div>
                        <p style={{ fontSize: "var(--text-xs)", fontWeight: 600, color: T.sub, marginBottom: 6 }}>Telefon</p>
                        <input type="tel" value={current.phone || ""} onChange={(e) => updateDraft({ phone: e.target.value })} style={inputStyle} />
                      </div>
                    </div>
                    <div>
                      <p style={{ fontSize: "var(--text-xs)", fontWeight: 600, color: T.sub, marginBottom: 8 }}>Region (hemmaregion)</p>
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
                    ...
                    <div>
                      <p style={{ fontSize: "var(--text-xs)", fontWeight: 600, color: T.sub, marginBottom: 8 }}>Kan jobba i dessa regioner</p>
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
                        {regions.map((r) => {
                    ...
                    <div>
                      <p style={{ fontSize: "var(--text-xs)", fontWeight: 600, color: T.sub, marginBottom: 10 }}>Kontaktinställningar</p>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", columnGap: 32 }}>
                    {[
                      ...
                      <div key={k} style={{ padding: "12px 0", borderBottom: "1px solid var(--line)" }}>
                        <p style={{ fontSize: "var(--text-2xs)", color: "var(--ink-400)", marginBottom: 4, fontWeight: 600 }}>{k}</p>
                        <p style={{ fontSize: "var(--text-base)", color: "var(--ink-900)", fontWeight: 500, fontFamily: mono ? "var(--mono)" : "inherit" }}>{v}</p>
                      </div>
                    ))}
                  </div>
                )}
              ...
              <div style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 14, padding: "22px 24px" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
                  <p style={{ fontSize: "var(--text-2xs)", fontWeight: 800, letterSpacing: 1.2, textTransform: "uppercase", color: "var(--ink-500)" }}>Körkort &amp; certifikat</p>
                  <button onClick={startEditing} style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "6px 12px", borderRadius: 8, background: "transparent", border: "1px solid var(--line-2)", fontSize: "var(--text-xs)", fontWeight: 600, color: "var(--ink-700)", cursor: "pointer", fontFamily: "inherit" }}>
                    + Lägg till
                  </button>
                </div>
                {editing ? (
                  <div style={{ marginBottom: -8 }}>
                    <p style={{ fontSize: "var(--text-2xs)", color: T.sub, marginBottom: 14 }}>Redigera nedan och spara</p>
                  </div>
                ) : null}
                {editing ? (
                  <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                    <div>
                      <p style={{ fontSize: "var(--text-xs)", fontWeight: 600, color: T.sub, marginBottom: 10 }}>Körkortsbehörighet</p>
                        ...
                        {licenseTypes.map((l) => {
                          ...
                          return (
                            <button key={l.value} onClick={() => toggleLicense(l.value)} style={{
                              ...
                              fontWeight: 800, fontSize: "var(--text-lg)", fontFamily: T.font, transition: "all .15s",
                    ...
                    {CERT_GROUPS.map((grp) => {
                      ...
                      return (
                        <div key={grp.id}>
                          <p style={{ fontSize: "var(--text-xs)", fontWeight: 600, color: T.sub, marginBottom: 8 }}>{grp.label}</p>
                          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                            {groupCerts.map((ct) => {
                              ...
                              return (
                                <div key={ct.value} style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                                  <button onClick={() => toggleCertificate(ct.value)} style={{
                                    padding: "7px 14px", borderRadius: 8, fontSize: "var(--text-xs)", fontWeight: 600, cursor: "pointer",
                                  ...
                                  {selected && (
                                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                      <span style={{ fontSize: "var(--text-xs)", color: T.muted }}>Utgår:</span>
                                      <input
                                        type="date"
                                        value={expiry}
                                        ...
                                        style={{
                                          ...
                                          color: T.text, fontSize: "var(--text-xs)", fontFamily: T.font, cursor: "pointer",
                                        }}
                                      />
                  ...
                  <>
                    ...
                    {(current.certificates || []).length > 0 ? (
                      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                          ...
                          return (
                            <div key={cid} style={{
                              ...
                              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                                ...
                                <span style={{ fontSize: "var(--text-base)", fontWeight: 700, color: "var(--ink-900)" }}>{certLabel}</span>
                              </div>
                              <span style={{ fontSize: "var(--text-sm)", color: warn ? dotColor : "var(--ink-500)", fontWeight: warn ? 600 : 500, fontFamily: !warn && st ? "var(--mono)" : "var(--font)" }}>
                        ...
                        {certWarnings.some((c) => c.status.days < 0) && (
                          <div style={{ marginTop: 8, padding: "12px 16px", borderRadius: 10, background: "var(--danger-tint)", border: "1px solid rgba(185,28,59,0.18)" }}>
                            <p style={{ fontSize: "var(--text-sm)", color: "var(--danger)", fontWeight: 600, marginBottom: 4 }}>Certifikat har gått ut</p>
                            <p style={{ fontSize: "var(--text-xs)", color: "var(--ink-500)", marginBottom: 8 }}>Du missar matchningar tills det är förnyat.</p>
                            <button onClick={startEditing} style={{ fontSize: "var(--text-xs)", fontWeight: 700, color: "var(--danger)", background: "none", border: "none", cursor: "pointer", fontFamily: "inherit", padding: 0 }}>
                      ...
                      <div style={{ padding: 16, borderRadius: 10, border: `1.5px dashed var(--line-2)`, textAlign: "center" }}>
                        <p style={{ fontSize: "var(--text-sm)", color: T.muted }}>Inga certifikat tillagda</p>
                        <button onClick={startEditing} style={{ fontSize: "var(--text-xs)", color: T.amber, background: "none", border: "none", cursor: "pointer", fontFamily: T.font, marginTop: 6 }}>+ Lägg till certifikat</button>
                      </div>
                    )}
                  </>
                )}
              ...
              {editing && (
                <>
                  <div style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 14, padding: "22px 24px" }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
                      <p style={{ fontSize: "var(--text-2xs)", fontWeight: 800, letterSpacing: 1.2, textTransform: "uppercase", color: "var(--ink-500)" }}>Tillgänglighet</p>
                    </div>
                    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                      <div>
                        <p style={{ fontSize: "var(--text-xs)", fontWeight: 600, color: T.sub, marginBottom: 8 }}>Status</p>
                        <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
                          {AVAIL.map((a) => (
                            <button key={a.value} onClick={() => updateDraft({ availability: a.value })} style={{
                              padding: "8px 16px", borderRadius: 9, fontSize: "var(--text-sm)", fontWeight: 500, cursor: "pointer",
                              fontFamily: T.font,
                      ...
                      {!current.isGymnasieelev && (
                        <div>
                          <p style={{ fontSize: "var(--text-xs)", fontWeight: 600, color: T.sub, marginBottom: 8 }}>Anställningsform</p>
                          <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
                      ...
                      <div>
                        <p style={{ fontSize: "var(--text-xs)", fontWeight: 600, color: T.sub, marginBottom: 6 }}>Tillgänglig från (valfritt)</p>
              ...
              <div style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 14, padding: "22px 24px" }}>
                <div style={{ marginBottom: 16 }}>
                  <p style={{ fontSize: "var(--text-2xs)", fontWeight: 800, letterSpacing: 1.2, textTransform: "uppercase", color: "var(--ink-500)" }}>Om mig</p>
                </div>
                <textarea
                  value={current.summary || ""}
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginTop: 6 }}>
                  <p style={{ fontSize: "var(--text-2xs)", color: T.muted }}>Minst {SUMMARY_MIN_LENGTH} tecken</p>
                  <p style={{ fontSize: "var(--text-2xs)", color: T.muted }}>{(current.summary || "").length}/400</p>
                </div>
              </div>
              ...
              <div style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 14, padding: "22px 24px" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
                  <p style={{ fontSize: "var(--text-2xs)", fontWeight: 800, letterSpacing: 1.2, textTransform: "uppercase", color: "var(--ink-500)" }}>Erfarenhet</p>
                  <button onClick={() => { startEditing(); setAddingExp(true); }} style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "6px 12px", borderRadius: 8, background: "transparent", border: "1px solid var(--line-2)", fontSize: "var(--text-xs)", fontWeight: 600, color: "var(--ink-700)", cursor: "pointer", fontFamily: "inherit" }}>
                    + Lägg till
                  </button>
                </div>
                ...
                {(current.experience || []).length === 0 && !addingExp ? (
                  <div style={{ padding: 24, border: `1.5px dashed var(--line-2)`, borderRadius: 12, textAlign: "center" }}>
                    <p style={{ fontSize: "var(--text-base)", color: T.sub, marginBottom: 6 }}>Ingen erfarenhet tillagd ännu</p>
                    <p style={{ fontSize: "var(--text-xs)", color: T.muted, lineHeight: 1.5 }}>Jobbhistorik är det åkerier tittar på först.</p>
                  </div>
                ) : (
                      ...
                      editingExpId === exp.id ? (
                        ...
                        <div key={exp.id} style={{
                          ...
                          <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6 }}>
                            <span style={{ fontSize: "var(--text-xs)", color: "var(--ink-500)", fontWeight: 600, fontFamily: "var(--mono)", whiteSpace: "nowrap", paddingTop: 4 }}>
                              {formatYearRange(exp)}
                            </span>
                            {editing && (
                              <div style={{ display: "flex", gap: 5 }}>
                                <button onClick={() => setEditingExpId(exp.id)} style={{ width: 28, height: 28, borderRadius: 7, border: `1px solid var(--line-2)`, background: T.card, color: T.sub, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "var(--text-sm)" }}>✎</button>
                                <button onClick={() => removeExperience(exp.id)} style={{ width: 28, height: 28, borderRadius: 7, border: "1px solid rgba(239,68,68,0.2)", background: "var(--danger-tint)", color: T.red, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "var(--text-sm)" }}>×</button>
                              </div>
                            )}
                          </div>
                        </div>
            ...
            <aside style={{ display: "flex", flexDirection: "column", gap: 18, position: "sticky", top: 80 }}>
              ...
              <div style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 14, padding: "22px 24px" }}>
                ...
                <p style={{ fontSize: "var(--text-sm)", color: "var(--ink-500)", marginBottom: 14, fontWeight: 500 }}>
                ...
                {!editing && displayScore < 100 && (
                  <button onClick={startEditing} style={{ marginTop: 14, width: "100%", padding: "8px", borderRadius: 8, border: "none", background: "var(--green-tint)", color: "var(--green-text)", fontSize: "var(--text-xs)", fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>Stärk profilen →</button>
                )}
              </div>
              ...
              {driverMarket && driverMarket.jobsInRegion >= 5 ? (
                <div style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 14, padding: "22px 24px" }}>
                  <div style={{ marginBottom: 16 }}>
                    <p style={{ fontSize: "var(--text-2xs)", fontWeight: 800, letterSpacing: 1.2, textTransform: "uppercase", color: "var(--ink-500)" }}>
                  ...
                  <p style={{ fontSize: "var(--text-xs)", color: "var(--ink-500)", marginTop: 14, lineHeight: 1.5 }}>Andel aktiva jobb som kräver respektive behörighet i din region.</p>
                </div>
              ) : (
                <div style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 14, padding: "22px 24px" }}>
                  <div style={{ marginBottom: 16 }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                    {[{ label: "CE-körkort", pct: 78 }, { label: "YKB", pct: 62 }, { label: "ADR", pct: 31 }, { label: "Truck B", pct: 24 }].map((m) => (
                      <div key={m.label}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 6 }}>
                          <span style={{ fontSize: "var(--text-sm)", color: "var(--ink-700)", fontWeight: 600 }}>{m.label}</span>
                          <span style={{ fontSize: "var(--text-sm)", color: "var(--ink-900)", fontWeight: 700, fontFamily: "var(--mono)" }}>{m.pct}%</span>
                        </div>
                        <div style={{ height: 4, borderRadius: 2, background: "var(--paper-2)", overflow: "hidden" }}>
                          <div style={{ height: "100%", width: `${m.pct}%`, background: "var(--green)", opacity: 0.4 + (m.pct / 100) * 0.6, borderRadius: 2 }} />
              ...
              {user?.id && (
                <div style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 14, padding: "22px 24px" }}>
                  <div style={{ marginBottom: 12 }}>
                    <p style={{ fontSize: "var(--text-2xs)", fontWeight: 800, letterSpacing: 1.2, textTransform: "uppercase", color: "var(--ink-500)" }}>Din profillänk</p>
                  </div>
            ...
            <div>
              {matchedJobsLoading ? (
                <div style={{ padding: 40, textAlign: "center" }}>
                  <p style={{ fontSize: "var(--text-base)", color: T.muted }}>Hämtar matchande jobb…</p>
                </div>
                        ...
                        >
                          <div style={{
                            ...
                            <div style={{
                              ...
                              display: "flex", alignItems: "center", justifyContent: "center",
                              fontSize: "var(--text-xl)", fontWeight: 700, color: T.primary, flexShrink: 0,
                            }}>
                              {(job.companyName || job.company?.name || "?").charAt(0)}
                            </div>
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4, flexWrap: "wrap" }}>
                                <p style={{ fontWeight: 700, fontSize: "var(--text-base)", color: T.text }}>{job.title || job.bransch || "Chaufförstjänst"}</p>
                              </div>
                              <p style={{ fontSize: "var(--text-xs)", color: T.sub, marginBottom: 8 }}>
                ...
                <div style={{ padding: 40, textAlign: "center", border: `1.5px dashed ${T.border2}`, borderRadius: 14 }}>
                  <p style={{ fontSize: "var(--text-base)", color: T.sub, marginBottom: 8 }}>Inga matchande jobb hittades</p>
                  <p style={{ fontSize: "var(--text-sm)", color: T.muted, lineHeight: 1.6 }}>
                  ...
                  <button onClick={() => setTab("profil")} style={{
                    ...
                    background: T.primary, color: "#fff", fontSize: "var(--text-sm)", fontWeight: 600,
                    cursor: "pointer", fontFamily: T.font,
          ...
          <div style={{ maxWidth: 800 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(3,1fr)", gap: 14, marginBottom: 24 }}>
                  {[
                    ...
                    <Card key={label} style={{ textAlign: "center", padding: "24px" }}>
                      ...
                      <p style={{ fontSize: "var(--text-xs)", color: T.sub, lineHeight: 1.4, marginBottom: hint ? 8 : 0 }}>{label}</p>
                      {hint && <p style={{ fontSize: "var(--text-2xs)", color: T.muted, lineHeight: 1.4 }}>{hint}</p>}
                    </Card>
                  ))}
                </div>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 20 }}>
                    {profileStats.recommendations.map((r, i) => (
                      <div key={i} style={{
                        ...
                        <span style={{ fontSize: "var(--text-sm)", color: T.sub, lineHeight: 1.55 }}>{r.text}</span>
                      </div>
                    ))}
                  </div>
                )}
            ...
            {!profileTips ? (
              <button
                ...
                style={{
                  padding: "10px 22px", borderRadius: 9, border: "none",
                  background: T.pDim, color: "var(--green-text)", fontSize: "var(--text-sm)", fontWeight: 600,
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 20 }}>
                {profileTips.map((tip, i) => (
                  <div key={i} style={{
                    ...
                    <span style={{ fontSize: "var(--text-sm)", color: T.sub, lineHeight: 1.55 }}>{tip}</span>
                  </div>
                ))}
              </div>
            )}
            ...
            <div style={{
              ...
              <p style={{ fontSize: "var(--text-base)", fontWeight: 700, color: T.amber, marginBottom: 8 }}>
      ...
      {editing && (
        <div style={{
          ...
          {profileSaveError && (
            <span style={{ fontSize: "var(--text-xs)", color: T.red, marginRight: 8 }}>{profileSaveError}</span>
          )}
          <button onClick={cancelEditing} style={{
            padding: "9px 20px", borderRadius: 9, border: "1px solid var(--line-2)",
            background: "var(--paper-2)", color: T.sub, fontSize: "var(--text-sm)", fontWeight: 600,
            cursor: "pointer", fontFamily: T.font,
          }}>Avbryt</button>
          <button onClick={saveProfile} disabled={profileSaving} style={{
            ...
            color: "#fff",
            fontSize: "var(--text-sm)", fontWeight: 700, cursor: "pointer", fontFamily: T.font,
