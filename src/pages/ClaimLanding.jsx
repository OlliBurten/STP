  ...
  if (loading) {
    return (
      <main style={{ minHeight: "100vh", background: "var(--paper)", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ fontSize: 15, color: "var(--ink-400)" }}>Laddar...</div>
      </main>
    );
  }
    ...
    return (
      <main style={{ minHeight: "100vh", background: "var(--paper)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
        <div style={{ maxWidth: 420, textAlign: "center" }}>
          ...
          <p style={{ fontSize: 15, color: "var(--ink-500)", marginBottom: 24 }}>{error}</p>
    ...
    return (
      <main style={{ minHeight: "100vh", background: "var(--paper)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
        <div style={{ maxWidth: 520, textAlign: "center" }}>
          ...
          <p style={{ fontSize: 15, color: "var(--ink-500)", lineHeight: 1.65, marginBottom: 24 }}>
          ...
          <button
            ...
            style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "13px 24px", borderRadius: 10, background: "var(--green)", color: "#fff", fontWeight: 800, border: "none", cursor: "pointer", fontSize: 14.5 }}
          >
    ...
    return (
      <main style={{ minHeight: "100vh", background: "var(--paper)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
        <div style={{ maxWidth: 480, textAlign: "center" }}>
          ...
          <h1 style={{ fontSize: 24, fontWeight: 800, color: "var(--ink-900)", marginBottom: 10 }}>{data.companyName} är redan anslutet</h1>
          <p style={{ fontSize: 15, color: "var(--ink-500)", marginBottom: 24 }}>Det här åkeriet har redan tagit över sina annonser på STP.</p>
          <Link to="/login" style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "13px 24px", borderRadius: 10, background: "var(--green)", color: "#fff", fontWeight: 800, textDecoration: "none", fontSize: 14.5 }}>
    ...
    <main style={{ minHeight: "100vh", background: "var(--paper)", paddingBottom: 80 }}>
      ...
      <div style={{ background: "var(--card)", borderBottom: "1px solid var(--line)", padding: "0 32px" }}>
        <div style={{ maxWidth: 1040, margin: "0 auto", height: 56, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <Link to="/login" style={{ fontSize: 13.5, fontWeight: 600, color: "var(--ink-500)", textDecoration: "none" }}>
      ...
      <div style={{ maxWidth: 760, margin: "0 auto", padding: "40px 24px 0" }}>
        ...
        <div style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 16, padding: "36px 40px", boxShadow: "var(--sh-sm)", marginBottom: 20, textAlign: "center" }}>
          ...
          <div style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "4px 12px", borderRadius: 999, background: "var(--amber-tint)", border: "1px solid var(--amber-tint-2)", color: "var(--amber-text)", fontSize: 12, fontWeight: 700, letterSpacing: 0.8, textTransform: "uppercase", marginBottom: 14 }}>
          ...
          <p style={{ fontSize: 15.5, color: "var(--ink-500)", lineHeight: 1.65, maxWidth: 520, margin: "0 auto 28px" }}>
            {data.applicationCount > 0
          ...
          {isCompany ? (
            <>
              <p style={{ fontSize: 14, color: "var(--ink-600)", marginBottom: 14 }}>
              ...
              <button
                ...
                disabled={activating}
                style={{ display: "inline-flex", alignItems: "center", gap: 9, padding: "15px 32px", borderRadius: 12, background: "var(--green)", color: "#fff", fontSize: 16, fontWeight: 800, border: "none", cursor: activating ? "not-allowed" : "pointer", boxShadow: "0 4px 16px rgba(31,95,92,0.3)", opacity: activating ? 0.7 : 1 }}
              >
            ...
            <>
              <button
                ...
                style={{ display: "inline-flex", alignItems: "center", gap: 9, padding: "15px 32px", borderRadius: 12, background: "var(--green)", color: "#fff", fontSize: 16, fontWeight: 800, border: "none", cursor: "pointer", boxShadow: "0 4px 16px rgba(31,95,92,0.3)", letterSpacing: -0.2 }}
              >
                Skapa gratis konto och se kandidaterna
                <Icon n="arrow" s={15} c="#fff" />
              </button>
              <div style={{ marginTop: 16, fontSize: 13, color: "var(--ink-400)" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginBottom: 20 }}>
            {[
              ...
              <div key={s.label} style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 12, padding: "18px 16px", textAlign: "center", boxShadow: "var(--sh-sm)" }}>
                <div style={{ fontSize: 28, fontWeight: 900, color: "var(--ink-900)", fontFamily: "var(--mono)", letterSpacing: -1, lineHeight: 1, marginBottom: 6 }}>{s.value}</div>
        ...
        <div style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 14, overflow: "hidden", boxShadow: "var(--sh-sm)", marginBottom: 20 }}>
          <div style={{ padding: "16px 24px", borderBottom: "1px solid var(--line)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: "var(--ink-700)" }}>Era annonser på STP</div>
            <div style={{ fontSize: 12, color: "var(--ink-400)" }}>{data.jobCount} {data.jobCount === 1 ? "annons" : "annonser"}</div>
          </div>
          {data.jobs.slice(0, 8).map((job, i) => (
            <div key={job.id} style={{ padding: "14px 24px", borderBottom: i < Math.min(data.jobs.length, 8) - 1 ? "1px solid var(--line)" : "none", display: "flex", alignItems: "center", gap: 14 }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 14, fontWeight: 700, color: "var(--ink-900)", marginBottom: 3 }}>{job.title}</div>
                <div style={{ fontSize: 12.5, color: "var(--ink-500)", display: "flex", gap: 8, flexWrap: "wrap" }}>
              ...
              {(job.license || []).map((l) => (
                <span key={l} style={{ padding: "3px 8px", borderRadius: 99, background: "var(--green)", color: "#fff", fontSize: 11, fontWeight: 700, flexShrink: 0 }}>{l}</span>
              ))}
            </div>
          ))}
          {data.jobCount > 8 && (
            <div style={{ padding: "12px 24px", textAlign: "center", fontSize: 13, color: "var(--ink-400)", background: "var(--paper-2)" }}>
        ...
        <div style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 14, padding: "24px 28px", boxShadow: "var(--sh-sm)", marginBottom: 20 }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: "var(--ink-500)", textTransform: "uppercase", letterSpacing: 1.2, marginBottom: 18 }}>Så här fungerar det</div>
          {[
            ...
            <div key={s.n} style={{ display: "flex", gap: 16, padding: "12px 0", borderBottom: i < arr.length - 1 ? "1px solid var(--line)" : "none", alignItems: "flex-start" }}>
              <div style={{ width: 28, height: 28, borderRadius: 99, background: "var(--green)", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 800, flexShrink: 0, marginTop: 1, fontFamily: "var(--mono)" }}>{s.n}</div>
              <div>
                <div style={{ fontSize: 14.5, fontWeight: 700, color: "var(--ink-900)", marginBottom: 3 }}>{s.t}</div>
                <div style={{ fontSize: 13, color: "var(--ink-500)", lineHeight: 1.5 }}>{s.s}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        {!isCompany && (
          <div style={{ textAlign: "center", marginTop: 32 }}>
            <button
              type="button"
              onClick={handleRegister}
              style={{ display: "inline-flex", alignItems: "center", gap: 9, padding: "15px 32px", borderRadius: 12, background: "var(--green)", color: "#fff", fontSize: 15.5, fontWeight: 800, border: "none", cursor: "pointer", boxShadow: "0 4px 16px rgba(31,95,92,0.3)" }}
            >
