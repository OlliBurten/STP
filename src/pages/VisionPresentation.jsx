...
const label = { fontSize: 11, fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", color: "var(--amber-text)" };
const h2style = { fontSize: 26, fontWeight: 800, color: "var(--ink-900)", letterSpacing: -0.5, margin: "12px 0 14px" };
const body = { fontSize: 14, color: "var(--ink-500)", lineHeight: 1.7, maxWidth: 640, margin: 0 };
    ...
    <main style={{ minHeight: "100vh", background: "var(--paper)" }}>
      ...
      <div style={{ background: "var(--card)", borderBottom: "1px solid var(--line)", padding: "80px 24px 64px" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <span style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "var(--success-tint)", border: "1px solid var(--success)", borderRadius: 99, padding: "4px 12px", fontSize: 11, fontWeight: 700, color: "var(--success)", letterSpacing: "0.08em", marginBottom: 24 }}>
          ...
          <p style={{ fontSize: 16, color: "var(--ink-500)", maxWidth: 580, lineHeight: 1.7, margin: "0 0 10px" }}>
          ...
          <p style={{ fontSize: 14, color: "var(--ink-400)", maxWidth: 580, lineHeight: 1.7, margin: "0 0 32px" }}>
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            <Link to="/jobb" style={{ display: "inline-block", background: "var(--green)", color: "#fff", padding: "11px 22px", borderRadius: 10, fontSize: 14, fontWeight: 800, textDecoration: "none" }}>
              Se plattformen live
            </Link>
            <Link to="/kontakt" style={{ display: "inline-block", border: "1px solid var(--line)", color: "var(--ink-700)", padding: "11px 22px", borderRadius: 10, fontSize: 14, fontWeight: 600, textDecoration: "none" }}>
              Kontakta oss
            </Link>
          </div>
        </div>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "48px 24px 80px", display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 12, marginTop: 24 }}>
            {PROBLEMS.map(({ title, text }) => (
              <div key={title} style={{ background: "var(--paper-2)", border: "1px solid var(--line)", borderRadius: 14, padding: "18px 20px" }}>
                <p style={{ fontSize: 14, fontWeight: 700, color: "var(--ink-900)", marginBottom: 6 }}>{title}</p>
                <p style={{ fontSize: 13, color: "var(--ink-500)", lineHeight: 1.6, margin: 0 }}>{text}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={card}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 8, marginTop: 24 }}>
            {LIVE_FEATURES.map((f) => (
              <div key={f} style={{ display: "flex", gap: 10, alignItems: "flex-start", background: "var(--success-tint)", border: "1px solid var(--success)", borderRadius: 10, padding: "12px 14px", fontSize: 13, color: "var(--ink-700)" }}>
                <CheckIcon />
                <span>{f}</span>
              </div>
            ))}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 12, marginTop: 24 }}>
            {SEGMENTS.map(({ label: seg, color, bg, border, text }) => (
              <div key={seg} style={{ background: bg, border: `1px solid ${border}`, borderRadius: 16, padding: "20px 22px" }}>
                <span style={{ display: "inline-block", background: "transparent", color, border: `1px solid ${border}`, borderRadius: 99, padding: "3px 10px", fontSize: 11, fontWeight: 700, marginBottom: 12 }}>{seg}</span>
                <p style={{ fontSize: 13, color: "var(--ink-500)", lineHeight: 1.6, margin: 0 }}>{text}</p>
              </div>
            ))}
          </div>
        ...
        <div style={card}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 32, marginTop: 24 }}>
            {[
              ...
              <div key={title}>
                <p style={{ fontSize: 14, fontWeight: 700, color: "var(--ink-900)", marginBottom: 16 }}>{title}</p>
                <ul style={{ display: "flex", flexDirection: "column", gap: 10, listStyle: "none", padding: 0, margin: 0 }}>
                  {items.map((s) => (
                    <li key={s} style={{ display: "flex", gap: 8, fontSize: 13, color: "var(--ink-500)", lineHeight: 1.5 }}>
                      <ArrowIcon />
                      <span>{s}</span>
                    </li>
                  ))}
        ...
        <div style={card}>
          ...
          <ul style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 24, listStyle: "none", padding: 0 }}>
            {ROADMAP.map((item) => (
              <li key={item} style={{ display: "flex", gap: 12, alignItems: "flex-start", background: "var(--paper-2)", border: "1px solid var(--line)", borderRadius: 10, padding: "14px 16px", fontSize: 13, color: "var(--ink-500)" }}>
        ...
        <div style={{ background: "var(--green-tint)", border: "1px solid rgba(31,95,92,0.2)", borderRadius: 20, padding: "40px" }}>
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            <Link to="/kontakt" style={{ display: "inline-block", background: "var(--green)", color: "#fff", padding: "11px 22px", borderRadius: 10, fontSize: 14, fontWeight: 800, textDecoration: "none" }}>
              Ta kontakt
            </Link>
            <Link to="/jobb" style={{ display: "inline-block", border: "1px solid var(--line)", color: "var(--ink-700)", padding: "11px 22px", borderRadius: 10, fontSize: 14, fontWeight: 600, textDecoration: "none" }}>
              Se plattformen live
            </Link>
          </div>
