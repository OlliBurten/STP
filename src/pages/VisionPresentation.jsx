...
const card = { background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 20, padding: "36px 40px" };
const label = { fontSize: 11, fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", color: "#F5A623" };
const h2style = { fontSize: 26, fontWeight: 800, color: "#f0faf9", letterSpacing: -0.5, margin: "12px 0 14px" };
const body = { fontSize: 14, color: "rgba(240,250,249,0.55)", lineHeight: 1.7, maxWidth: 640, margin: 0 };

function CheckIcon() {
  return (
    <svg viewBox="0 0 20 20" fill="currentColor" style={{ width: 16, height: 16, color: "#4ade80", flexShrink: 0, marginTop: 2 }}>
...
function ArrowIcon() {
  return (
    <svg viewBox="0 0 20 20" fill="currentColor" style={{ width: 14, height: 14, color: "#F5A623", flexShrink: 0, marginTop: 3 }}>
...
export default function VisionPresentation() {
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#060f0f", marginTop: "-64px" }}>

      {/* Hero */}
      <div style={{ background: "linear-gradient(135deg, #0a1818 0%, #060f0f 100%)", borderBottom: "1px solid rgba(255,255,255,0.06)", padding: "120px 24px 64px" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <span style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "rgba(74,222,128,0.12)", border: "1px solid rgba(74,222,128,0.2)", borderRadius: 99, padding: "4px 12px", fontSize: 11, fontWeight: 700, color: "#4ade80", letterSpacing: "0.08em", marginBottom: 24 }}>
            <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#4ade80" }} />
            Live sedan april 2026
          </span>
          <h1 style={{ fontSize: "clamp(28px, 5vw, 48px)", fontWeight: 900, color: "#f0faf9", letterSpacing: -1.5, lineHeight: 1.15, maxWidth: 700, margin: "0 0 20px" }}>
            Sveriges matchningsplattform för yrkesförare och transportföretag.
          </h1>
          <p style={{ fontSize: 16, color: "rgba(240,250,249,0.6)", maxWidth: 580, lineHeight: 1.7, margin: "0 0 10px" }}>
          ...
          <p style={{ fontSize: 14, color: "rgba(240,250,249,0.4)", maxWidth: 580, lineHeight: 1.7, margin: "0 0 32px" }}>
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            <Link to="/jobb" style={{ display: "inline-block", background: "#F5A623", color: "#000", padding: "11px 22px", borderRadius: 10, fontSize: 14, fontWeight: 800, textDecoration: "none" }}>
              Se plattformen live
            </Link>
            <Link to="/kontakt" style={{ display: "inline-block", border: "1px solid rgba(255,255,255,0.15)", color: "rgba(240,250,249,0.8)", padding: "11px 22px", borderRadius: 10, fontSize: 14, fontWeight: 600, textDecoration: "none" }}>
              Kontakta oss
            </Link>
          </div>
        </div>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "48px 24px 80px", display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        <div style={card}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 12, marginTop: 24 }}>
            {PROBLEMS.map(({ title, text }) => (
              <div key={title} style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 14, padding: "18px 20px" }}>
                <p style={{ fontSize: 14, fontWeight: 700, color: "#f0faf9", marginBottom: 6 }}>{title}</p>
                <p style={{ fontSize: 13, color: "rgba(240,250,249,0.5)", lineHeight: 1.6, margin: 0 }}>{text}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={card}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 8, marginTop: 24 }}>
            {LIVE_FEATURES.map((f) => (
              <div key={f} style={{ display: "flex", gap: 10, alignItems: "flex-start", background: "rgba(74,222,128,0.04)", border: "1px solid rgba(74,222,128,0.1)", borderRadius: 10, padding: "12px 14px", fontSize: 13, color: "rgba(240,250,249,0.7)" }}>
                <CheckIcon />
                <span>{f}</span>
              </div>
            ))}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 12, marginTop: 24 }}>
            {SEGMENTS.map(({ label: seg, color, bg, border, text }) => (
              <div key={seg} style={{ background: bg, border: `1px solid ${border}`, borderRadius: 16, padding: "20px 22px" }}>
                <span style={{ display: "inline-block", background: `${color}22`, color, border: `1px solid ${color}44`, borderRadius: 99, padding: "3px 10px", fontSize: 11, fontWeight: 700, marginBottom: 12 }}>{seg}</span>
                <p style={{ fontSize: 13, color: "rgba(240,250,249,0.6)", lineHeight: 1.6, margin: 0 }}>{text}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={card}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 32, marginTop: 24 }}>
            {[
              ...
              <div key={title}>
                <p style={{ fontSize: 14, fontWeight: 700, color: "#f0faf9", marginBottom: 16 }}>{title}</p>
                <ul style={{ display: "flex", flexDirection: "column", gap: 10, listStyle: "none", padding: 0, margin: 0 }}>
                  {items.map((s) => (
                    <li key={s} style={{ display: "flex", gap: 8, fontSize: 13, color: "rgba(240,250,249,0.6)", lineHeight: 1.5 }}>
                      <ArrowIcon />
                      <span>{s}</span>
                    </li>
                  ))}
        ...
        <div style={card}>
          ...
          <ul style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 24, listStyle: "none", padding: 0 }}>
            {ROADMAP.map((item) => (
              <li key={item} style={{ display: "flex", gap: 12, alignItems: "flex-start", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 10, padding: "14px 16px", fontSize: 13, color: "rgba(240,250,249,0.6)" }}>
                <svg viewBox="0 0 20 20" fill="currentColor" style={{ width: 16, height: 16, color: "rgba(240,250,249,0.3)", flexShrink: 0, marginTop: 2 }}>
        ...
        {/* CTA */}
        <div style={{ background: "linear-gradient(135deg, rgba(31,95,92,0.3) 0%, rgba(31,95,92,0.1) 100%)", border: "1px solid rgba(31,95,92,0.35)", borderRadius: 20, padding: "40px" }}>
          <p style={{ ...label, color: "rgba(125,211,200,0.7)" }}>Kontakt & samarbete</p>
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            <Link to="/kontakt" style={{ display: "inline-block", background: "#F5A623", color: "#000", padding: "11px 22px", borderRadius: 10, fontSize: 14, fontWeight: 800, textDecoration: "none" }}>
              Ta kontakt
            </Link>
            <Link to="/jobb" style={{ display: "inline-block", border: "1px solid rgba(255,255,255,0.15)", color: "rgba(240,250,249,0.8)", padding: "11px 22px", borderRadius: 10, fontSize: 14, fontWeight: 600, textDecoration: "none" }}>
              Se plattformen live
            </Link>
          </div>
          <p style={{ fontSize: 12, color: "rgba(240,250,249,0.3)", marginTop: 20 }}>
