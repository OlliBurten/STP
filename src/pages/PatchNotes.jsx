    ...
    <main style={{ background: "var(--paper)", minHeight: "100vh", paddingTop: 96 }}>
      ...
      <section style={{ maxWidth: 720, margin: "0 auto", padding: "0 24px 80px" }}>
        ...
        <div style={{ maxWidth: 560, marginBottom: 48 }}>
          <span style={{ display: "inline-flex", alignItems: "center", gap: 6, borderRadius: 20, background: "var(--success-tint)", border: "1px solid var(--success)", padding: "4px 12px", fontSize: 12, fontWeight: 700, color: "var(--success)", letterSpacing: "0.5px" }}>
          ...
          <p style={{ fontSize: 16, color: "var(--ink-500)", lineHeight: 1.7, margin: 0 }}>
              ...
              <div style={{ padding: "22px 28px 18px", borderBottom: "1px solid var(--line)" }}>
                <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent: "space-between", gap: 10, marginBottom: 12 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    {index === 0 && (
                      <span style={{ display: "inline-flex", borderRadius: 20, background: "var(--success-tint)", color: "var(--success)", padding: "4px 10px", fontSize: 11, fontWeight: 700, border: "1px solid var(--success)" }}>
                        Senaste
                      </span>
                    )}
                  </div>
                  <p style={{ fontSize: 13, color: "var(--ink-300)", margin: 0 }}>{note.date}</p>
                </div>
              ...
              <ul style={{ listStyle: "none", margin: 0, padding: "18px 28px", display: "flex", flexDirection: "column", gap: 12 }}>
                {note.items.map((item) => (
                  <li key={item} style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                    ...
                    <span style={{ fontSize: 14, color: "var(--ink-500)", lineHeight: 1.65 }}>{item}</span>
                  </li>
                ))}
              </ul>
            </article>
        ...
        <p style={{ marginTop: 40, textAlign: "center", fontSize: 14, color: "var(--ink-300)" }}>
